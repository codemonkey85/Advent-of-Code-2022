...
closest to the snack storage, a giant Rock Paper Scissors tournament is already in
progress.

Rock Paper Scissors is a game between two players. Each game contains many
rounds; in each round, the players each simultaneously choose one of Rock, Paper, or
...
column is what your opponent is going to play: A for Rock, B for Paper, and C for
Scissors. The second column--" Suddenly, the Elf is called away to help with someone's
tent.
...
is the score for the shape you selected (1 for Rock, 2 for Paper, and 3 for
Scissors) plus the score for the outcome of the round (0 if you lost, 3 if the round was
...
const string A = nameof(A); // rock
const string B = nameof(B); // paper
const string C = nameof(C); // scissors
const string X = nameof(X); // rock, lose
...
void GetYourTotalScore(string input)
{
    ...
    var playerRounds = (
        ...
        let p2Choice = choices[1]
        select new PlayerRound(GetChoice(p1Choice), GetChoice(p2Choice)).Result).ToList();
    Console.WriteLine(playerRounds.Sum(r => r.P2Points));
}
...
GetYourTotalScorePart2(puzzleInput);
...
void GetYourTotalScorePart2(string input)
{
    ...
    var playerRounds = (
        ...
        let p1Choice = choices[0]
        let p2Choice = choices[1]
        let playerChoices = GetChoicePart2(p1Choice, p2Choice)
        select new PlayerRound(playerChoices.P1Choice, playerChoices.P2Choice).Result).ToList();
    Console.WriteLine(playerRounds.Sum(r => r.P2Points));
}

#region Stuff

static IChoice GetChoice(string choiceString) => choiceString switch
{
    A or X => new Rock(),
    B or Y => new Paper(),
    C or Z => new Scissors(),
    _ => throw new ArgumentOutOfRangeException(nameof(choiceString), choiceString, null)
};

static (IChoice P1Choice, IChoice P2Choice) GetChoicePart2(string p1ChoiceString, string p2ChoiceString)
{
    IChoice p1Choice = p1ChoiceString switch
    {
        A => new Rock(),
        B => new Paper(),
        C => new Scissors(),
        _ => throw new ArgumentOutOfRangeException(nameof(p1ChoiceString), p1ChoiceString, null)
    };
    IChoice p2Choice = p1Choice switch
    {
        Rock => p2ChoiceString switch
        {
            X => new Scissors(), // lose
            Y => new Rock(), // draw
            Z => new Paper(), // win
        ...
        Scissors => p2ChoiceString switch
        {
            X => new Paper(), // lose
            Y => new Scissors(), // draw
            Z => new Rock(), // win
...
internal record Paper(int Points = 2) : IChoice;
...
{
    private const int Loser = 0;
    private const int Draw = 3;
    private const int Winner = 6;

    public RoundResult Result => P1 switch
    {
        Rock => P2 switch
        {
